...
const PARTY_ARTICLES = [
  {
    slug: "party-of-the-greens",
    partyName: "Партия на ЗЕЛЕНИТЕ",
    title: "Партия на Зелените: профил, приоритети и позиции",
    description: "Профил на Партия на Зелените с акцент върху екологичния, гражданския и проевропейския ѝ облик.",
    history: [
      "Партия на Зелените е една от по-дълго присъстващите екологично ориентирани формации в България и се свързва с опити да се представят теми като устойчиво развитие, опазване на природата и граждански контрол върху публичните решения.",
      "Макар да не е сред големите парламентарни играчи, партията периодично участва в избори и присъства в по-широкото поле на зелени и граждански формации."
    ],
    content: [
      "Партията има проевропейски и екологично ориентиран профил.",
      "Основният ѝ политически облик е по-свързан с устойчиво развитие, качество на средата и гражданска чувствителност, отколкото с класически твърд идеологически сблъсък по оста ляво-дясно.",
      "За избиратели, за които околната среда и устойчивите политики имат значение, такава формация може да бъде полезна референтна точка."
    ]
  },
  {
    slug: "direct-democracy",
    partyName: "ПРЯКА ДЕМОКРАЦИЯ",
    title: "Пряка демокрация: профил, приоритети и позиции",
    description: "Профил на Пряка демокрация с акцент върху гражданското участие и прякото влияние върху решенията.",
    history: [
      "Партия „Пряка демокрация“ е по-малка формация, чийто профил се свързва с идеи за по-пряко участие на гражданите в политическия процес и с критика към класическия партиен модел.",
      "Формацията не е сред трайните парламентарни играчи, но присъства като част от по-широкото поле на партии, които поставят процедурния въпрос за демокрацията в центъра на идентичността си."
    ],
    content: [
      "Централната идея тук не е толкова конкретна икономическа доктрина, колкото повече граждански контрол и механизми за директно влияние върху политическите решения.",
      "Това прави профила ѝ по-специфичен и по-малко класически в сравнение с традиционните големи партии."
    ]
  },
  {
    slug: "third-march",
    partyName: "ТРЕТИ МАРТ",
    title: "Трети март: профил, приоритети и позиции",
    description: "Профил на коалицията Трети март с акцент върху национално-суверенисткия и по-скептичен външнополитически профил.",
    history: [
      "Коалицията „Трети март“ се появява в контекста на по-широки търсения за национално-ориентиран и суверенистки политически формат, който използва силна историческа символика.",
      "Публичният образ на формацията е свързан с теми за държавен интерес, идентичност и по-предпазливо отношение към външната зависимост."
    ],
    content: [
      "Формацията е по-близо до национално-ориентираните и евроскептични среди, отколкото до твърдо интеграционисткия спектър.",
      "Подходът ѝ по външнополитически и енергийни теми е по-скоро държавно защитен и суверенистки."
    ]
  },
  {
    slug: "velichie",
    partyName: "ВЕЛИЧИЕ",
    title: "Величие: профил, приоритети и позиции",
    description: "Подробен профил на Величие с акцент върху национално-популисткия профил, изборното развитие и публичните позиции.",
    history: [
      "Партия „Величие“ е основана през 2023 г. и бързо привлече обществено внимание като нов националистически и популистки проект, свързван с Ивелин Михайлов. Първоначалното ѝ развитие е силно обвързано с местната база във Ветрино и с по-широк антисистемен разказ.",
      "През юни 2024 г. партията успява да влезе в парламента, а след изборите през октомври 2024 г. остава под прага по официалните резултати, преди по-късно да се върне в Народното събрание след преизчисления и решение на Конституционния съд. Това превърна Величие в една от по-необичайните и бурно развиващи се нови формации в последните години."
    ],
    content: [
      "Величие се възприема като националистическа и популистка формация с подчертан антисистемен тон и критика към установения политически модел.",
      "По темите ЕС, Русия и сигурност партията се разполага по-близо до евроскептичния и суверенистки спектър, отколкото до твърдо проевропейските сили.",
      "Профилът ѝ е по-ясно политико-културен и протестен, отколкото детайлно разработен като класическа секторна управленска програма.",
      "На избирателно ниво Величие привлича хора, които търсят по-рязка критика към елитите и по-твърд национален тон.",
      "Формацията вече има реална парламентарна история, което я отличава от съвсем новите и все още изцяло извънпарламентарни проекти."
    ]
  },
  {
    slug: "nepokorna-bulgaria",
    partyName: "НАЦИОНАЛНО ДВИЖЕНИЕ НЕПОКОРНА БЪЛГАРИЯ",
    title: "Непокорна България: профил, приоритети и позиции",
    description: "Профил на Непокорна България с акцент върху център-лявото позициониране и връзката с Корнелия Нинова.",
    history: [
      "Непокорна България се появява първо като движение в края на 2024 г., а през 2025 г. се оформя и като партия, свързана с Корнелия Нинова след раздялата ѝ с БСП.",
      "Историята на проекта е пряко свързана с кризите в левицата и с опита да се изгради нов център-ляв политически дом за избиратели, които не се разпознават в актуалното развитие на БСП."
    ],
    content: [
      "Непокорна България има по-скоро център-ляв и социално ориентиран профил, с акцент върху ролята на държавата, социалната защита и по-предпазлив външнополитически тон.",
      "Формацията е естествено разположена по-близо до традиционната социална и по-държавно ориентирана политика, отколкото до нискоданъчната десница.",
      "Тъй като е сравнително нов проект, профилът ѝ все още се развива, но вече има ясно разпознаваем лидерски и идеен контекст.",
      "За избиратели, които търсят лявоцентристка алтернатива извън БСП, формацията може да изглежда по-близка."
    ]
  },
  {
    slug: "glas-naroden",
    partyName: "ГЛАС НАРОДЕН",
    title: "Глас народен: профил, приоритети и позиции",
    description: "Профил на Глас народен с акцент върху протестния и нетрадиционен политически стил.",
    history: [
      "Глас народен е основана през 2013 г. от Светослав Витков и се превръща в една от по-нестандартните партии в българската политика, с по-ироничен и антисистемен публичен образ.",
      "Още в първите си участия формацията успява да привлече внимание като протестна и нетрадиционна алтернатива, макар да не се превръща в трайно парламентарна сила."
    ],
    content: [
      "Глас народен има по-граждански, протестен и нетрадиционен стил в сравнение с класическите партийни структури.",
      "Политическият ѝ образ е по-малко идеологически твърд и повече свързан с критика към системата и публичния политически език.",
      "Формацията не е сред големите играчи, но има разпознаваема история и собствена идентичност в българския партиен живот."
    ]
  },
  {
    slug: "gerb-sds",
    ...
    history: [
      "Коалицията ГЕРБ-СДС е създадена през 2019 г. като предизборен съюз между ГЕРБ и СДС. Тя стъпва върху по-ранната история на ГЕРБ, която е основана през 2006 г. около Бойко Борисов и бързо се утвърждава като водеща сила в българския политически живот.",
      "След 2009 г. ГЕРБ няколко пъти оглавява правителства, а партньорството със СДС оформя по-ясно дясноцентристкия и проевропейски профил на коалицията. Историята на ГЕРБ-СДС е тясно свързана с управленския опит на ГЕРБ и с опита за обединяване на част от традиционното дясно пространство около обща листа."
    ],
    content: [
      ...
      "По темите Русия, НАТО и сигурност коалицията стои в рамките на общата евроатлантическа ориентация. В публичния образ на формацията това обикновено означава подкрепа за НАТО, за координация със съюзниците и за по-дистанциран подход към руското влияние.",
      ...
      "Избирател, който търси проевропейска ориентация, предвидима данъчна среда, по-ниски преки данъци и по-прагматичен стил на управление, вероятно ще открие по-високо съвпадение с позициите на ГЕРБ-СДС."
    ]
  },
  {
    ...
    history: [
      "БСП е сред най-старите политически традиции в България, като официалният сайт на партията подчертава корени от 1891 г. Съвременната Българска социалистическа партия приема настоящото си име през 1990 г. след промените в края на комунистическия период.",
      "Настоящата коалиционна форма БСП – ОБЕДИНЕНА ЛЕВИЦА стъпва върху по-ранни изборни формати на левицата и беше преименувана в този вид през 2024 г. Историята ѝ съчетава дългата организационна традиция на БСП с опит за по-широко обединение на леви и социално ориентирани формации."
    ],
    content: [
      ...
      "По отношение на Европейския съюз БСП-ОЛ не е извън европейската рамка, но обикновено използва по-предпазлив и по-социално оцветен език.",
      ...
      "Избиратели, които търсят по-силна социална държава, по-голяма роля на държавата в икономиката и по-умерен външнополитически тон, могат да намерят по-високо съвпадение с БСП – ОБЕДИНЕНА ЛЕВИЦА."
    ]
  },
  {
    slug: "pp-db",
    ...
    history: [
      "Коалицията ПП-ДБ е оформена през февруари 2023 г. като съюз между „Продължаваме промяната“ и „Демократична България“. Самата „Продължаваме промяната“ е основана през 2021 г. от Кирил Петков и Асен Василев, а „Демократична България“ обединява по-стари дясноцентристки и либерални формации.",
      "Историята на ПП-ДБ е тясно свързана с вълната от антикорупционни очаквания и с опита за консолидиране на проевропейското реформаторско пространство. След изборите през 2023 г. коалицията участва и в управленски формат с ротационно правителство, което допълнително я утвърди като централен играч в реформаторския спектър."
    ],
    content: [
  ...
  },
  {
    ...
    history: [
      "ДПС е основано на 4 януари 1990 г. от Ахмед Доган в ранния посткомунистически период. Партията се появява в контекста на възстановяването на правата на турското и мюсюлманското население след т.нар. възродителен процес и постепенно се утвърждава като постоянен участник в българския парламентарен живот.",
      "През годините ДПС се превръща в една от най-устойчивите организационно партии в страната и многократно участва или влияе върху формулирането на парламентарни мнозинства. Историята му е свързана както с представителството на малцинствени общности, така и с по-широк прагматичен политически профил."
    ],
    content: [
      ...
      "Избиратели, които търсят по-прагматична политика, проевропейска рамка, предвидима данъчна среда и фокус върху регионалното развитие, могат да открият съвпадение с ДПС."
    ]
  },
  {
    slug: "aps",
    ...
    history: [
      "Алианс за права и свободи се появи през 2024 г. след разцеплението в ДПС и се оформи като политически дом на крилото, останало лоялно на Ахмед Доган. В началото формацията беше позната основно като парламентарна и изборна група около т.нар. доганистко крило.",
      "През 2026 г. Алиансът предприе и стъпка към регистрация като самостоятелна партия. Поради кратката си история и ограничената публична програмна база, профилът му все още е по-малко структуриран от този на утвърдените партии в българската политика."
    ],
    content: [
      ...
      "Когато няма достатъчно достъпни програмни документи и последователни публични позиции, най-честният подход е това да бъде обозначено ясно.",
      "За избирателя това означава, че ако иска да направи добре информиран избор относно формацията, е полезно да търси допълнителни официални източници, изявления, интервюта и програмни документи.",
      "В подобни случаи тестът и сравнението могат да покажат ограничено съвпадение или по-неутрална позиция.",
  ...
  },
  {
    ...
    history: [
      "„Възраждане“ е основана през 2014 г., а неин председател от самото начало е Костадин Костадинов. Партията тръгва като по-малка националистическа формация, но в следващите години постепенно разширява присъствието си чрез силно антисистемна и суверенистка реторика.",
      "По-големият политически пробив на „Възраждане“ идва в периода на честите избори след 2021 г., когато партията успява да влезе и да увеличава представителството си в парламента. Историята ѝ е свързана с натрупване на подкрепа сред избиратели, критични към ЕС, НАТО и към установения политически модел."
    ],
    content: [
      ...
      "В икономиката и държавната роля формацията често подкрепя по-силен национален контрол и защита на местната икономика.",
  ...
  },
  {
    ...
    history: [
      "„Има такъв народ“ е регистрирана през февруари 2020 г. около Слави Трифонов, след като предишният опит за регистрация на партия под името „Няма такава държава“ беше спрян заради спор около символиката. Още с първото си сериозно участие в избори през 2021 г. формацията се превърна във важен фактор в периода на политическа нестабилност.",
      "Историята на ИТН е силно свързана с протестната енергия от началото на десетилетието и с персоналния образ на Слави Трифонов. Партията мина през периоди както на силен изборен резултат, така и на по-скромно представяне, но остана разпознаваем играч в парламентарния процес."
    ],
    content: [
  ...
  },
  {
    ...
    history: [
      "„Синя България“ е сравнително нова коалиция, създадена през 2024 г. Тя стъпва върху по-ранни опити за консолидиране на дясното пространство, включително формати като „Национално обединение на десницата“ и местни коалиции като „Синя София“.",
      "Историята на „Синя България“ е по-кратка от тази на традиционните партии, но политическият ѝ смисъл е свързан с опит за ново обединяване на дясноцентристки, антикомунистически и проевропейски среди. Това я поставя в по-широката традиция на българската демократична десница, макар и в нов организационен формат."
    ],
    content: [
      "Синя България се възприема като формация, която се стреми да заеме по-ясно дясноцентристко пространство.",
      "В икономиката подобен профил обикновено означава подкрепа за предприемачеството и по-ниска данъчна тежест.",
      "По темата ЕС Синя България стои по-близо до проевропейския спектър.",
      "По отношение на Русия и сигурността формацията по-скоро изглежда по-далеч от модели на сближаване с Москва.",
      "Тъй като публичната информация за някои конкретни секторни политики е по-ограничена, профилът на формацията следва да се разглежда предпазливо.",
  ...
  },
  {
    ...
    history: [
      "МЕЧ е нова партия, създадена през 2024 г. от Радостин Василев след разрив с предишното му политическо обкръжение. Още от началото формацията се позиционира като остро антисистемен и конфронтационен политически проект.",
      "Поради кратката си история МЕЧ все още няма дълъг управленски или парламентарен архив, но успя бързо да привлече внимание с персонализиран стил, силна реторика срещу статуквото и опит да капитализира протестен и наказателен вот. Историята ѝ тепърва се оформя през резултатите и организационното развитие."
    ],
    content: [
      "МЕЧ се свързва с по-остър и по-конфронтационен политически стил, който често търси енергията на протестния вот.",
      "Антисистемният тон може да бъде привлекателен за хора, които искат по-рязък политически език.",
      "Когато публичната програмна конкретика е ограничена, е трудно една формация да бъде позиционирана с висока увереност по всички ключови политики.",
      "За избирателя е важно да различава между силен политически тон и детайлна програмна рамка.",
      "МЕЧ може да бъде по-близо до хора, които търсят по-остър политически сблъсък със статуквото.",
...
,
  {
    ...
    title: "Прогресивна България: профил, приоритети и позиции",
    description: "Профил на коалицията Прогресивна България с акцент върху социална политика и център-ляво позициониране.",
    officialSite: "https://progresivnabulgaria.bg",
    programLink: "https://progresivnabulgaria.bg/programa",
    history: [
      "Коалицията „Прогресивна България“ е създадена през март 2026 г. по инициатива на бившия президент Румен Радев. Форматът е оформен като център-ляво политическо обединение, насочено към участие в парламентарните избори през 2026 г.",
      "В коалицията участват няколко по-малки партии със социалдемократическа ориентация. Проектът се позиционира като нов политически субект, който цели да привлече избиратели с по-социална ориентация и критично отношение към съществуващия политически модел."
    ],
    content: [
      "Прогресивна България се позиционира в център-лявото политическо пространство и поставя акцент върху социалната справедливост и държавната роля.",
      "В икономическата политика се очаква по-голяма роля на публичните инвестиции и подкрепа за социални системи.",
      "По отношение на външната политика формацията се стреми към балансиран подход в рамките на европейските институции.",
      "В здравеопазването и образованието се подчертава необходимостта от по-голямо публично финансиране.",
      "Формацията цели да привлече избиратели, които търсят по-социално ориентирана политика.",
      "Като нов проект, профилът ѝ все още се оформя чрез публични позиции и програми."
    ]
  },
  {
    slug: "rumen-radev",
    partyName: "РУМЕН РАДЕВ",
    title: "Румен Радев: политически профил",
    description: "История и политически профил на Румен Радев.",
    officialSite: "https://president.bg",
    programLink: "https://president.bg/program",
    history: [
      "Румен Радев е български военен пилот и политик, който става широко известен като командир на Военновъздушните сили на България.",
      "През 2016 г. печели президентските избори и заема поста президент на Република България от 2017 до 2026 г. След приключване на мандата си участва в създаването на нов политически проект."
    ],
    content: [
      "Политическият профил на Румен Радев е свързан с публични позиции по теми като корупцията, управлението и външната политика.",
      "По време на президентството си често заема активна позиция по ключови национални въпроси.",
      "След края на мандата си се включва в политически процеси, насочени към създаване на нови политически инициативи.",
      "Подкрепата му традиционно идва от избиратели, търсещи по-силна президентска институция.",
      "Профилът му комбинира военен и политически опит.",
      "Ролята му в българската политика остава значима и след президентството."
    ]
  }
];
...
const BLOG_POSTS = [
  ...
  {
    slug: "kak-raboti-mashinnoto-glasuvane",
    title: "Как работи машинното гласуване в България",
    excerpt: "Кратко и ясно обяснение какво прави машината, как се потвърждава вотът и какво е важно да знаете.",
    content: [
      "Машинното гласуване често създава несигурност, защото много хора не го използват ежедневно и се притесняват дали вотът им ще бъде отчетен правилно. На практика системата е създадена така, че да води избирателя стъпка по стъпка и да намали част от най-честите грешки при попълване на бюлетина.",
      "Важно е да се знае, че машината не е просто екран за натискане, а част от официалния изборен процес, при който има ясни правила, проверка на избора и отпечатан контролен разпис. Това дава на избирателя възможност да провери избора си преди окончателното пускане.",
      "За много хора най-полезното е да разглеждат машинното гласуване като технически улеснен начин за подаване на валиден глас, а не като нещо мистериозно или трудно. Колкото по-добре човек разбира процеса, толкова по-малко напрежение има в изборния ден.",
      "Подобни теми са важни и за сайтове като Izborite.info, защото информираният избор не зависи само от партиите, а и от това гражданите да са спокойни как реално упражняват вота си."
    ]
  },
  {
    slug: "kak-se-razpredelyat-mandatite",
    title: "Как се разпределят мандатите след изборите",
    excerpt: "Обяснение защо процентът гласове не винаги изглежда равен на броя депутати.",
    content: [
      "Много избиратели се изненадват, когато видят, че процентът гласове на една партия не съвпада напълно с броя на депутатите, които тя получава. Причината е, че изборната система разпределя мандатите по конкретни правила, които включват райони, бариера за влизане и метод за изчисляване.",
      "Това означава, че не всеки глас се превръща механично в едно и също представителство. Различните райони имат различен брой мандати, а формации под определен праг изобщо не участват в окончателното разпределение. Така резултатът често изглежда по-сложен от самите проценти в изборната вечер.",
      "За избирателя е полезно да разбира този процес, защото така по-лесно може да тълкува какво реално означава един резултат и защо малки разлики в проценти понякога водят до по-големи политически последствия."
    ]
  },
  {
    slug: "kakvo-pravi-narodno-sabranie",
    title: "Какво всъщност прави Народното събрание",
    excerpt: "Защо изборът на парламент има значение далеч отвъд дневните политически скандали.",
    content: [
      "Парламентът не е само място за спорове между партии, а основната институция, която приема закони, контролира правителството и определя посоката на държавната политика. Затова парламентарните избори имат пряко значение за ежедневния живот на гражданите.",
      "Много от темите, които се обсъждат в теста на Izborite.info – данъци, социални плащания, енергетика, сигурност и институционални реформи – минават именно през решения на Народното събрание. Това е причината изборът на депутати да е толкова важен.",
      "Когато човек разбира ролята на парламента, по-лесно преценява защо програмите на партиите имат значение и защо не е достатъчно да се следят само лидерските образи и кампанийни лозунги."
    ]
  },
  {
    slug: "kak-da-proverya-izbiratelnata-si-sekciya",
    title: "Как да проверя избирателната си секция преди вота",
    excerpt: "Практични насоки защо е добре да проверите секцията си по-рано, а не в последния момент.",
    content: [
      "Една от най-полезните стъпки преди изборния ден е да проверите предварително в коя секция гласувате. Това спестява време, объркване и излишно напрежение, особено ако сте сменяли адрес или не сте гласували скоро.",
      "Подобна проверка е важна и защото понякога избирателите разчитат, че ще гласуват на познато място, а междувременно има промени в организацията. Предварителната яснота улеснява участието в изборния процес.",
      "Тези на пръв поглед малки практически теми също са част от информираното участие в демократичния процес. Не е достатъчно само да знаете за кого бихте гласували – важно е и да сте подготвени как реално да упражните правото си."
    ]
  },
  {
    slug: "zashto-politicheskiyat-profil-ne-e-prisuda",
    title: "Защо резултатът от политически тест не е присъда",
    excerpt: "Как да четете процента съвпадение разумно и без да превръщате теста в окончателен избор.",
    content: [
      "Политическият тест може да е много полезен, но процентът съвпадение не бива да се чете като автоматична препоръка. Той е ориентир, изграден върху ограничен набор от въпроси, а реалният политически избор често включва и фактори като доверие, лидерство, управленски стил и конкретен контекст.",
      "Понякога човек може да има сравнително висок резултат с партия, към която няма доверие по други причини. В други случаи съвпадението може да е по-ниско, но една конкретна тема да е толкова важна, че да има по-голяма тежест от общия процент.",
      "Най-разумният подход е тестът да се използва като карта, а не като заповед. Той помага да се подреди мисленето и да се насочи вниманието към партии, които си струва да бъдат разгледани по-задълбочено."
    ]
  }
];
