...
const PARTY_ARTICLES = [
  {
    ...
    description:
      "Подробен преглед на ГЕРБ-СДС по теми като икономика, данъци, ЕС, Русия, сигурност, енергетика и управление.",
    content: [
  ...
  },
  {
    ...
    description:
      "Подробен профил на БСП – ОБЕДИНЕНА ЛЕВИЦА с акцент върху социалната държава, данъците, ЕС, Русия и икономиката.",
    content: [
  ...
  },
  {
    ...
    description:
      "Подробна статия за ПП-ДБ с акцент върху антикорупция, ЕС, еврозона, данъчна рамка и институционални реформи.",
    content: [
  ...
  },
  {
    ...
    description:
      "Подробен профил на ДПС по теми като икономика, региони, ЕС, сигурност и прагматичен политически подход.",
    content: [
  ...
  },
  {
    ...
    description:
      "Подробен профил на Възраждане по теми като ЕС, Русия, държавна роля, миграция и национален суверенитет.",
    content: [
  ...
  },
  {
    ...
    description:
      "Подробен профил на ИТН по теми като прагматичен център, държавно управление, ЕС, сигурност и данъчна стабилност.",
    content: [
...
const BLOG_POSTS = [
  ...
  },
  {
    ...
    content: [
      ...
      "В България допълнително се намесват теми като ЕС, Русия, антикорупция и отношение към институциите. Затова една партия може да бъде икономически по-дясна, но външнополитически по-умерена, или обратно. Именно затова е полезно да се гледа многомерно сравнение, а не само един етикет."
    ]
  },
  {
...
function navigateTo(path, setCurrentPage, setSelectedParty, setSelectedBlog) {
  ...
  if (path.startsWith("/blog/")) {
    ...
    setSelectedBlog(BLOG_POSTS.find((p) => p.slug === slug) || null);
    setSelectedParty(null);
    return;
  }

  if (path === "/partii") {
    setCurrentPage("party-hub");
    setSelectedParty(null);
    setSelectedBlog(null);
    return;
  }

  if (path.startsWith("/partii/")) {
    const slug = path.replace("/partii/", "");
    const article = PARTY_ARTICLES.find((p) => p.slug === slug);
    setCurrentPage("party-page");
    setSelectedParty(parties.find((p) => p.name === article?.partyName) || null);
    setSelectedBlog(null);
    return;
  }
